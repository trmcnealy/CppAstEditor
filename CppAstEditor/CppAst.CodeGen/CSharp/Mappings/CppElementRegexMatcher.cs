...
namespace CppAst.CodeGen.CSharp
{
    public class CppElementRegexMatcher : CppElementMatcher
    {
        ...
        public CppElementRegexMatcher(string regexString)
        {
            ...
            string[] regexTxtParts = _regexString.Split(new[] { "::" }, StringSplitOptions.RemoveEmptyEntries);
            foreach (string regexTxtPart in regexTxtParts)
            {
                try
                {
                    Regex regex = new Regex($"^{regexTxtPart}$");
        ...
        {
            CppElement iterator = cppElement;
            int matchIndex = 0;
            ...
            while (iterator != null)
            {
                if (iterator is ICppMember member)
                {
                    ...
                    Regex regexPart = _regexParts[matchIndex];
                    Match match = regexPart.Match(member.Name);
                    if (match.Success)
                    {
                        if (matches == null)
                        {
