...
namespace CppAst.CodeGen.CSharp
{
    public static class CSharpElementExtensions
    {
        ...
        {
            CSharpContainerList<CSharpElement> members = element.Members;
            for (int i = 0; i < members.Count; i++)
            {
                CSharpElement elementMember = members[i];
        ...
        public static void DumpAttributesTo(this ICSharpAttributesProvider element, CodeWriter writer)
        {
            ...
            foreach (CSharpAttribute attr in element.GetAttributes())
            {
                writer.Write("[");
                attr.DumpTo(writer);
                writer.Write("]");
        ...
        public static void DumpContextualAttributesTo(this CSharpElement element, CodeWriter writer, bool inline = false, CSharpAttributeScope? scopeOverride = null)
        {
            ...
            if (inline)
            {
                ...
                foreach (CSharpAttribute attr in provider.GetContextualAttributes())
                {
                    if (isFirst)
                    {
                        writer.Write("[");
