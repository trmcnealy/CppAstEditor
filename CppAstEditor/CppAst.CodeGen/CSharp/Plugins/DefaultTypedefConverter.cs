...
namespace CppAst.CodeGen.CSharp
{
    public class DefaultTypedefConverter : ICSharpConverterPlugin
    {
        ...
        {
            CppType elementType = cppTypedef.ElementType;

            if (DefaultFunctionTypeConverter.IsFunctionType(elementType, out CppFunctionType cppFunctionType))
            {
                return DefaultFunctionTypeConverter.ConvertNamedFunctionType(converter, cppFunctionType, context, cppTypedef);
            }

            bool isFromSystemIncludes = converter.IsFromSystemIncludes(cppTypedef);

            CSharpType csElementType = converter.GetCSharpType(elementType, context, true);

            bool noWrap = converter.Options.TypedefCodeGenKind == CppTypedefCodeGenKind.NoWrap && !converter.Options.TypedefWrapWhiteList.Contains(cppTypedef.Name);
            ...
            string csStructName = converter.GetCSharpName(cppTypedef, context);
            CSharpStruct csStruct = new CSharpStruct(csStructName)
            {
                CppElement = cppTypedef,
            };

            ICSharpContainer container = converter.GetCSharpContainer(cppTypedef, context);
            ...
            string attachedAttributes = string.Empty;
            string csElementTypeName = is_size_t ? "nint" : converter.ConvertTypeReferenceToString(csElementType, out attachedAttributes);
