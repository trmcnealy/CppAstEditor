...
namespace CppAst.CodeGen.CSharp
{
    ...
    public class DefaultFieldConverter : ICSharpConverterPlugin
    {
        ...
        public static CSharpElement ConvertField(CSharpConverter converter, CppField cppField, CSharpElement context)
        {
            ...
            bool isGlobalVariable = (!(cppField.Parent is CppClass) && !isConst) || cppField.StorageQualifier == CppStorageQualifier.Static;
            if (isGlobalVariable)
            {
                return null;
            }

            bool isParentClass = cppField.Parent is CppClass;

            ICSharpContainer csContainer = converter.GetCSharpContainer(cppField, context);

            bool isUnion = ((cppField.Parent as CppClass)?.ClassKind ?? CppClassKind.Struct) == CppClassKind.Union;

            string csFieldName = converter.GetCSharpName(cppField, (CSharpElement)csContainer);

            if (cppField.IsBitField)
            {
                CSharpBitField csBitFieldStorage = null;
                for (int index = csContainer.Members.Count - 1; index >= 0; index--)
                {
                    CSharpElement member = csContainer.Members[index];
                ...
                {
                    CppPrimitiveType canonicalType = (CppPrimitiveType)cppField.Type.GetCanonicalType();
                ...
                CSharpProperty csProperty = new CSharpProperty(csFieldName)
                {
                    CppElement = cppField,
                ...
                long bitmask = (1L << cppField.BitFieldWidth) - 1;
                string bitmaskStr = $"0b{Convert.ToString(bitmask, 2)}";
                string notBitMaskStr = Convert.ToString(~(bitmask << currentBitOffset), 2);
            ...
            string parentName = cppField.Parent is CppClass cppClass ? cppClass.Name : string.Empty;
            CSharpField csField = new CSharpField(csFieldName) { CppElement = cppField };
