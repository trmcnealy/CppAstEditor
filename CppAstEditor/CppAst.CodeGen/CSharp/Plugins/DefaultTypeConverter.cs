...
namespace CppAst.CodeGen.CSharp
{
    ...
    public class DefaultTypeConverter : ICSharpConverterPlugin
    {
        ...
        public static CSharpType GetCSharpType(CSharpConverter converter, CppType cppType, CSharpElement context, bool nested)
        {
            ...
            CSharpType csType = converter.FindCSharpType(cppType);
            if (csType != null)
            {
                return csType;
            }
            ...
            if (isPointer)
            {
                ...
                {
                    CSharpType pointedCSharpType = converter.GetCSharpType(elementType, context, true);

                    bool isParam = context is CSharpParameter;
                    bool isReturn = context is CSharpMethod;
                    if (!nested && (isParam || isReturn))
                    {
                        switch (elementType.TypeKind)
                        {
                            ...
                            case CppTypeKind.Qualified:
                                CppQualifiedType qualifiedType = (CppQualifiedType)elementType;
                            ...
                            case CppTypeKind.Primitive:
                                CppPrimitiveType cppPrimitive = (CppPrimitiveType)elementType;
                                ...
                                {
                                    csType = new CSharpPointerType(pointedCSharpType);//new CSharpRefType(CSharpRefKind.Ref, pointedCSharpType);
                                }
                                break;
                            case CppTypeKind.Pointer:
                                csType = isParam ? new CSharpPointerType(pointedCSharpType) : null;
                                break;
                        }
                    }
                    else
            ...
            else
            {
                switch (cppType.TypeKind)
                {
                    case CppTypeKind.Array:
                        ...
                        CppArrayType arrayType = (CppArrayType)cppType;
                        CppType arrayElementType = arrayType.ElementType;
                        ...
                        {
                            CSharpTypeWithAttributes fixedStrType = new CSharpTypeWithAttributes(CSharpPrimitiveType.String);
                        ...
                        else
                        {
                            ...
                            {
                                CSharpTypeWithMembers csParent = (CSharpTypeWithMembers)csField.Parent;
                                csParent.Modifiers |= CSharpModifiers.Unsafe;

                                CSharpType csArrayElementType = converter.GetCSharpType(arrayElementType, context, true);
                            ...
                            else
                            {
                                ...
                                CSharpTypeWithAttributes typeWithAttributes = new CSharpTypeWithAttributes(csType);
                                CSharpMarshalAttribute attr = new CSharpMarshalAttribute(CSharpUnmanagedKind.LPArray);
                                if (csArrayElementType is CSharpTypeWithAttributes csArrayElementTypeWithAttributes)
                                {
                                    CSharpMarshalAttribute marshalAttributeForArrayElementType = GetMarshalAttributeOrNull(csArrayElementTypeWithAttributes.Attributes);
                    ...
                    case CppTypeKind.Reference:
                        ...
                        break;
                    case CppTypeKind.Qualified:
                        CppQualifiedType qualifiedType = (CppQualifiedType)cppType;
        ...
        {
            foreach (CSharpAttribute cSharpAttribute in attributes)
            {
        ...
        public static CSharpType GetBoolType(CSharpConverter converter)
        {
            ...
            {
                CSharpTypeWithAttributes boolTypeWithMarshal = new CSharpTypeWithAttributes(CSharpPrimitiveType.Bool);
        ...
        public static CSharpType GetStringType(CSharpConverter converter)
        {
            ...
            {
                CSharpTypeWithAttributes boolTypeWithMarshal = new CSharpTypeWithAttributes(CSharpPrimitiveType.String);
