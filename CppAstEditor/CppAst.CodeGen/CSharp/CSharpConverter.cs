...
namespace CppAst.CodeGen.CSharp
{
    public sealed class CSharpConverter
    {
        ...
        public static CSharpCompilation Convert(List<string> files, CSharpConverterOptions options)
        {
            ...
            CSharpConverter converter = new CSharpConverter(options);
        ...
        private CSharpCompilation Run(Func<CppParserOptions, CppCompilation> parse)
        {
            ...
            {
                CppParserOptions preprocessingOptions = Options.Clone();
                preprocessingOptions.AdditionalArguments.Add("--preprocess");
                preprocessingOptions.ParseMacros = true;

                CppCompilation cppCompilationPreprocessed = parse(preprocessingOptions);
                ...
                if (!cppCompilationPreprocessed.HasErrors)
                {
                    ...
                    for (int i = _pipeline.AfterPreprocessing.Count - 1; i >= 0; i--)
                    {
                        AfterPreprocessingDelegate preprocess = _pipeline.AfterPreprocessing[i];
                        preprocess(this, cppCompilationPreprocessed, additionalHeaders);
                    }
                }
            }

            CppParserOptions cppOptions = Options.Clone();
            ...
            CppCompilation cppCompilation = parse(cppOptions);
        ...
        public object GetTagValueOrNull(string name)
        {
            ...
            Tags.TryGetValue(name, out object obj);
            return obj;
        }

        public T GetTagValueOrDefault<T>(string name)
        {
            object obj = GetTagValueOrNull(name);
        ...
        {
            StringWriter strWriter = (StringWriter)_csTempWriter.CurrentWriter;
            strWriter.GetStringBuilder().Length = 0;
            csType.DumpReferenceTo(_csTempWriter);
            string typeReferenceName = strWriter.ToString();
        ...
        {
            if (_mapCppToCSharp.TryGetValue(cppElement, out CSharpElement csElement)) return csElement;
            ...
            try
            {
                ...
                {
                    CSharpElement subContext = csElement ?? context;
                    ...
                    int childCount = 0;
                    foreach (ICppDeclaration nestedCppElement in container.Children())
                    {
        ...
        public CSharpComment GetCSharpComment(CppElement element, CSharpElement context)
        {
            ...
            for (int i = _pipeline.CommentConverters.Count - 1; i >= 0; i--)
            {
                ConvertCommentDelegate commentConverter = _pipeline.CommentConverters[i];
                CSharpComment csComment = commentConverter(this, element, context);
                if (csComment != null)
                {
                    return csComment;
                }
        ...
        public string GetCSharpName(CppElement member, CSharpElement context, string defaultName = null)
        {
            ...
            if (context == null) throw new ArgumentNullException(nameof(context));
            string name = null;
            for (int i = _pipeline.GetCSharpNameResolvers.Count - 1; i >= 0; i--)
            {
                GetCSharpNameDelegate del = _pipeline.GetCSharpNameResolvers[i];
        ...
        public CSharpType GetCSharpType(CppType cppType, CSharpElement context, bool nested = false)
        {
            ...
            CSharpType csType;
            for (int i = _pipeline.GetCSharpTypeResolvers.Count - 1; i >= 0; i--)
            {
                GetCSharpTypeDelegate getCSharpTypeDelegate = _pipeline.GetCSharpTypeResolvers[i];
        ...
        public void ApplyDefaultVisibility(ICSharpElementWithVisibility element, ICSharpContainer container)
        {
            ...
            CSharpVisibility elementVisibility = container is CSharpTypeWithMembers typeWithMembers ? typeWithMembers.Visibility : CSharpVisibility.None;
        ...
        {
            for (int i = _pipeline.ConvertBegin.Count - 1; i >= 0; i--)
            {
                GlobalProcessingDelegate converting = _pipeline.ConvertBegin[i];
                converting(this);
            }
        }
        ...
        {
            for (int i = _pipeline.ConvertEnd.Count - 1; i >= 0; i--)
            {
                GlobalProcessingDelegate converting = _pipeline.ConvertEnd[i];
                converting(this);
            }
        }
        ...
        {
            for (int i = _pipeline.Converting.Count - 1; i >= 0; i--)
            {
                ProcessBeforeConvertDelegate converting = _pipeline.Converting[i];
        ...
        {
            for (int i = _pipeline.Converted.Count - 1; i >= 0; i--)
            {
                ProcessAfterConvertDelegate converted = _pipeline.Converted[i];
        ...
        {
            for (int i = _pipeline.CompilationConverters.Count - 1; i >= 0; i--)
            {
                ConvertCompilationDelegate tryProcessCompilation = _pipeline.CompilationConverters[i];
                CSharpCompilation element = tryProcessCompilation(this, item, context);
                if (element != null)
                {
                    return element;
                }
        ...
        {
            for (int i = _pipeline.EnumConverters.Count - 1; i >= 0; i--)
            {
                ConvertEnumDelegate tryEnum = _pipeline.EnumConverters[i];
                CSharpElement element = tryEnum(this, item, context);
                if (element != null)
                {
                    return element;
                }
        ...
        {
            for (int i = _pipeline.EnumItemConverters.Count - 1; i >= 0; i--)
            {
                ConvertEnumItemDelegate tryEnumItem = _pipeline.EnumItemConverters[i];
                CSharpElement element = tryEnumItem(this, item, context);
                if (element != null)
                {
                    return element;
                }
        ...
        {
            for (int i = _pipeline.ClassConverters.Count - 1; i >= 0; i--)
            {
                ConvertClassDelegate tryProcessClass = _pipeline.ClassConverters[i];
                CSharpElement element = tryProcessClass(this, item, context);
                if (element != null)
                {
                    return element;
                }
        ...
        {
            for (int i = _pipeline.FieldConverters.Count - 1; i >= 0; i--)
            {
                ConvertFieldDelegate tryField = _pipeline.FieldConverters[i];
                CSharpElement element = tryField(this, item, context);
                if (element != null)
                {
                    return element;
                }
        ...
        //{
        //    for (int i = _pipeline.FunctionConverters.Count - 1; i >= 0; i--)
        //    {
        //        ConvertFunctionDelegate tryProcessFunction = _pipeline.FunctionConverters[i];
        //        CSharpElement element = tryProcessFunction(this, item, context);
        //        if (element != null)
        //        {
        //            return element;
        //        }
        ...
        private CSharpElement TryConvertFunction(CppFunction item, CSharpElement context)
        {
            for (int i = _pipeline.ExportToFunctionPointerConverters.Count - 1; i >= 0; i--)
            {
                ConvertFunctionDelegate tryProcessFunction = _pipeline.ExportToFunctionPointerConverters[i];
                CSharpElement           element            = tryProcessFunction(this, item, context);
                if (element != null)
                {
                    return element;
        ...
        {
            for (int i = _pipeline.FunctionTypeConverters.Count - 1; i >= 0; i--)
            {
                ConvertFunctionTypeDelegate tryProcessFunctionType = _pipeline.FunctionTypeConverters[i];
                CSharpElement element = tryProcessFunctionType(this, item, context);
                if (element != null)
                {
                    return element;
                }
        ...
        {
            for (int i = _pipeline.ParameterConverters.Count - 1; i >= 0; i--)
            {
                ConvertParameterDelegate tryProcessParameter = _pipeline.ParameterConverters[i];
                CSharpElement element = tryProcessParameter(this, item, index, context);
                if (element != null)
                {
                    return element;
                }
        ...
        {
            for (int i = _pipeline.TypedefConverters.Count - 1; i >= 0; i--)
            {
                ConvertTypedefDelegate tryProcessTypedef = _pipeline.TypedefConverters[i];
                CSharpElement element = tryProcessTypedef(this, item, context);
                if (element != null)
                {
                    return element;
                }
        ...
        public void AddUsing(ICSharpContainer container, string referenceName, string aliasName = null, bool isStatic = false)
        {
            ...
            CSharpContainerList<CSharpElement> members     = usingCompatibleContainer.Members;
            int                   insertIndex = 0;
            for (int i = 0; i < members.Count; i++, insertIndex++)
            {
                CSharpElement member = members[i];
        ...
        private void Register(CppElement cppElement, CSharpElement element)
        {
            ...
            if (_mapCppToCSharp.TryGetValue(cppElement, out CSharpElement csElement))
            {
        ...
        {
            if (cppElement == null) throw new ArgumentNullException(nameof(cppElement));
            _mapCppToCSharp.TryGetValue(cppElement, out CSharpElement csElement);
            return csElement;
        }
        ...
        public ICSharpContainer GetCSharpContainer(CppElement element, CSharpElement context)
        {
            if (!ReferenceEquals(element.Parent, CurrentCppCompilation) && !ReferenceEquals(element.Parent, CurrentCppCompilation.System))
            {
                ...
                CSharpElement nextContext = context;
                ...
                {
                    ICSharpContainer currentContainer = GetCSharpCurrentContainer();
        ...
        {
            for (int i = _pipeline.GetCSharpContainerResolvers.Count - 1; i >= 0; i--)
            {
                GetCSharpContainerDelegate getCSharpContainer = _pipeline.GetCSharpContainerResolvers[i];
                ICSharpContainer container = getCSharpContainer(this, element, context);
                if (container != null)
                {
                    return container;
                }
