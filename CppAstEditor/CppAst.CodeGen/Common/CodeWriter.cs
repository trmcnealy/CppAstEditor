...
namespace CppAst.CodeGen.Common
{
    public class CodeWriter
    {
        ...
        public virtual void PushFileOutput(UPath path, bool autoDispose = true)
        {
            ...
            UPath filePath = UPath.Combine(UPath.Root, path);
            UPath directory = filePath.GetDirectory();
            if (!directory.IsEmpty && !fileSystem.DirectoryExists(directory))
            {
                fileSystem.CreateDirectory(directory);
            }
            StreamWriter finalWriter = new StreamWriter(new BufferedStream(fileSystem.CreateFile(filePath)));
            PushOutput(finalWriter, autoDispose);
        }

        public virtual TextWriter PopOutput()
        {
            WriterOutput writerOutput = _backendWriters.Pop();
        ...
        public CodeWriter Write(string text)
        {
            ...
            TextWriter currentWriter = CurrentWriter;
            ...
            int firstEndOfLine = text.IndexOf('\n');
            int lastEndOfLine = text.LastIndexOf('\n');
            bool hasTrailingEndOfLine = text.EndsWith("\n");
            bool hasMultiLine = firstEndOfLine != lastEndOfLine || (firstEndOfLine >= 0 && !hasTrailingEndOfLine);

            if (hasMultiLine)
            {
                int startIndex = 0;
                bool isNextNewLine = true;
                int nextStartIndex = firstEndOfLine < 0 ? text.Length : firstEndOfLine + 1;
                ...
                while (true)
                {
                    ...
                    string subText = NormalizeLine(text.Substring(startIndex, nextStartIndex - startIndex));
            ...
            {
                string normalizedLineText = NormalizeLine(text);
