...
namespace CppAst.CodeGen.Common
{
    public class CodeWriterOptions
    {
        ...
        public object this[string tagName]
        {
            get
            {
                Tags.TryGetValue(tagName, out object obj);
                return obj;
            }
            set
            {
        ...
        {
            PhysicalFileSystem fs = new PhysicalFileSystem();
            SubFileSystem subfs = new SubFileSystem(fs, fs.ConvertPathFromInternal(Environment.CurrentDirectory));
            return subfs;
        }
    }
}